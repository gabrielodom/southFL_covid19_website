...
browCases_df <- read_csv(
	file = "FLDH_COVID19_cases_broward_20200705.csv"
) %>% 
	mutate(PropPositive = 100 * Positive / (Positive + Negative)) %>% 
...
```{r pb_read_data}
pbCases_df <- read_csv(
	file = "FLDH_COVID19_cases_palmbeach_20200705.csv"
) %>% 
	mutate(PropPositive = 100 * Positive / (Positive + Negative))
```
...
- **COVID-19 Test Results**: The number of positive and negative COVID-19 test results come from the Florida Department of Health: https://urldefense.proofpoint.com/v2/url?u=https-3A__floridahealthcovid19.gov_&d=DwIGaQ&c=lhMMI368wojMYNABHh1gQQ&r=ZzssdlRrRS7QXnhuTSa63A&m=f8ZNrPo_m9V7uCNqTwuan8vNnfBlAYcGcGCOGgeMMVg&s=IzqCiAbuSp5tMaA9tW7jZhfqf0g30QO7wcTXAeM6GGU&e= .
- **COVID-19 Hospital Census**: The hospitalization data is disseminated by Florida’s Agency for Health Care Administration: https://urldefense.proofpoint.com/v2/url?u=https-3A__ahca.myflorida.com_&d=DwIGaQ&c=lhMMI368wojMYNABHh1gQQ&r=ZzssdlRrRS7QXnhuTSa63A&m=f8ZNrPo_m9V7uCNqTwuan8vNnfBlAYcGcGCOGgeMMVg&s=Cw-5--hoyaiHrLroTX3WYB-3Iowk-1pR0SwQvnyIauo&e= .
