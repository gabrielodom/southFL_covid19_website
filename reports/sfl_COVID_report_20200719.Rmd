...
- Deaths up until two weeks ago (due to data imcompleteness)
...
southFloridaHospitalised_df <- 
	read_csv(
		file = "../../data/ESS_southFL_summary_20200726.csv"
	)
...
deathsbyday_df <- read_csv(
	"../../data/FLDH_COVID19_deathsbyday_bycounty_20200726.csv"
)
```
...
# Gabriel's Machine:
mdCases_df <- read_csv(
	file = "../../data/FLDH_COVID19_cases_miamidade_20200727.csv"
) 
...
nPositive_gg <- 
	...
	labs(
		...
		subtitle = paste(
			...
			"2020; Recent Linear Change =",
			round(mdRecentPCountSlope_num, 1)
		)
	) + 
...
browCases_df <- read_csv(
	file = "../../data/FLDH_COVID19_cases_broward_20200727.csv"
) %>% 
	mutate(PropPositive = 100 * Positive / (Positive + Negative)) %>% 
	mutate(PositivePer1k = (PropPositive / 100) * 1000) %>% 
...
```{r pb_read_data}
pbCases_df <- read_csv(
	file = "../../data/FLDH_COVID19_cases_palmbeach_20200727.csv"
) %>% 
	mutate(PropPositive = 100 * Positive / (Positive + Negative)) %>% 
	mutate(PositivePer1k = (PropPositive / 100) * 1000)
```
