...
nTotal_gg <- 
	...
	labs(
		...
		subtitle = paste0(
			...
			"; Average Two Week Change = ",
			round(recent_TestSlope_num, 1),
			" Tests Daily"
		),
...
The average number of COVID-19 tests performed each day has increased from about 2,000 in early April to over 12,500 currently. Over the past two weeks there was <b>`r adjective_tests`</b> of `r round(recent_TestSlope_num,1)`  `r adjective_tests_2` tests performed each day. Note: data plotted on a log scale.
...
There was a general decrease in the number of new positive test results from early April until around May 18th, the day Miami-Dade County reopened. Afterwards, this trend flattened out and then increased until late July before it started to decrease.

### Positive COVID-19 Tests within the Past Two Weeks

In addition to the overall trend of positive COVID-19 tests shown above, we also show an enhanced view of the last two weeks. <b> Over the past two weeks, there has been an average `r adjective_cases` of `r round(recentCasesSlope_num,0)` `r adjective_cases_2` cases reported each day.</b>
...
# Slope Adjectives: Additional or Fewer
adjective_slope_2 <- case_when(
	abs(recentSlope_num) < 0.0 ~ "additional",
	abs(recentSlope_num) > 0.0 ~ "fewer",
)

# Slope Adjectives: Increase or Decrease
adjective_slope_3 <- ifelse(recentSlope_num > 0, "increase", "decrease")
```
...
During the 14-day period from `r recentStartEnd_date[1]` to `r recentStartEnd_date[2]`, there was a <b> `r magnitude_slope` `r adjective_slope_3` </b> in the proportion of positive COVID-19 cases reported in Miami-Dade County. During this time, there was an average `r adjective_cases` of  `r round(recentSlope_num, 3)`% per day for new positive cases. Given a flat or increasing number of tests performed, the proportion of positive tests obtained is the **key metric** for examining COVID-19 trajectory.
...
ggplot(data = sflCases2_df) +
	...
	labs(
		...
		caption = "Solid line: GAM Predictor; Orange Triangle: Phase 1 Reopening, Red Triangles: Phase 2 and Phase 3 Reopenings",
		y = "Proportion (%) of Positive Cases"
	) +
...
## Available County Beds
We now show a census plot of available beds:
```{r capacity, warning = FALSE}
ggplot(
	data = miamidadeHospitalized_df
) +

	theme_bw() +
	theme(legend.position = "bottom") + 
	aes(x = Date, y = AvailableBeds) +
	scale_color_manual(
		values = c(
			"AvailableBeds" = "#ff7400"
		)
	) +
	
	labs(
		title = "Overall Miami-Dade Available Beds",
		subtitle = paste(
			"Miami Dade County;",
			format(startEndESS_date[1], "%d %B %y"),
			"to",
...
Currenty there are `r lastDay_df$AvailableBeds` available hospital beds in the county.
...
deathsbyday_df <- read_csv(
	file = "../../data/deaths/FLDH_COVID19_deathsbyday_bycounty_20210103.csv"
) %>% 
...
# Conclusion
During the past two weeks, there has been <b>`r adjective_tests`</b> of `r round(recent_TestSlope_num,1)` `r adjective_tests_2` tests performed each day. There was an <b>`r adjective_cases`</b> in the number of positive COVID-19 tests obtained by an average of `r round(recentCasesSlope_num, 0)` each day. There was a <b> `r adjective_slope_3` </b> in the proportion of positive COVID-19 tests by `r round(recentSlope_num, 3)` % each day. Finally, the number of deaths reported per day has been increasing slightly. There is now approximately an `r round(certDelay_num / 7, 1)` week delay in the reporting of deaths for Miami-Dade County.
