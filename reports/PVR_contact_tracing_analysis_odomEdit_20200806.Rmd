...
contactHeaders <-
	read_excel(
  "../../data/FLDH_contact_tracing_ExtendedQuestions_20200715.xlsx",
  n_max = 0
)
...
```{r recode_entries}
### 2. Re-Code Entries as Indicators  ###
...
# Apply RecodeChar function to dataframe to make NA values = 0; first 19 columns
#   are text-entry indicators from a "choose all that apply" question
exposure_df <- map_dfc(
  .x = newContact_df[1:19],
  .f = RecodeChar
)
```

```{r wrangle_ZIP_codes}
zip_char <- newContact_df$`Business Zip Code  (if applicable)`
# Changing a ZIP code from 333635 to 33635, assuming there was a typo
zip_char[which(zip_char == "333635")] <- "33635"

# Finds 49 zips instead of 46
# sum(str_detect(zip_char, "[0-9]{5}"), na.rm = TRUE)

zip2_char <- 
  zip_char %>% 
  # First find all the entries that have 5-digit numbers
  str_subset("[0-9]{5}") %>% 
  # Then from those entries extract only the 5-digit numbers (this will return
  #   all matches)
  str_extract_all("[0-9]{5}", simplify = TRUE) %>% 
  as.character()
```
...
# Graph 
ggplot(topExposure_df) +
  
  theme_bw() +
  theme(legend.position = "none") +
  aes(x = reorder(Setting, Cases), y = Cases, fill = Cases / N) +
  labs(
    x = "Location", y = "Number of Cases",
    fill = "Proportion of Total Cases",
    title = "Proportion of Total Cases Per Reported Exposure Location",
    caption = "This graph only includes relationships with proportion > 0.05."
  ) +
  scale_x_discrete(labels = function(x) stringr::str_trunc(x, 20)) +
  scale_fill_gradient(low = "blue", high = "red") +  
  
  geom_col() +
  geom_text(
    aes(label = paste0(100 * round(Cases / N, digits = 2), "%")), 
    color = "white", hjust = 1.1, size = 5
  ) +
...
The ZIP codes with the highest workplace traced counts are
```{r top_ZIPS}
sort(
  table(zip2_char, dnn = "Top ZIPs")[table(zip2_char) > 1],
  decreasing = TRUE
)
```

The patterns of exposure are
```{r}
# cor(exposure_df)
# This is too large to parse visually

# Which Rows and Columns of the Correlation Matrix should we keep?
keepRC_int <- exposure_df %>% 
  cor() %>% 
  abs() %>% 
  `>`(0.2) %>% 
  which(arr.ind = TRUE) %>%
  as_tibble(rownames = "Feature") %>%
  mutate(keep = abs(row - col) > 0) %>%
  filter(keep) %>% 
  select(row, col) %>% 
  unlist() %>% 
  unique() %>% 
...
exposure_cor <- cor(exposure_df)[keepRC_int, keepRC_int]
rownames(exposure_cor) <- stringr::str_trunc(rownames(exposure_cor), 15)
colnames(exposure_cor) <- stringr::str_trunc(colnames(exposure_cor), 15)

round(exposure_cor, 2)
```
...
relation_df <- 
  newContact_df %>% 
  select(`What is the relationship to the source? `) %>% 
...
ggplot(data = relation_df) +
  ...
  labs(
    ...
    y = "Number of Cases", 
    fill = "Proportion of Total Cases",
    title = "Proportion of Cases by Relationship to COVID-19 Transmitter",
    caption = "This graph only includes relationships with proportion > 0.05"
  ) +
  # scale_y_continuous(
  #   limits = c(0, 120),
  ...
  geom_col() +
  geom_text(
    aes(label = paste0(100 * round(n / sum(relation_df$n), digits = 2), "%")), 
    hjust = 1.25, 
    color = "white",
    size = 5
  ) +
