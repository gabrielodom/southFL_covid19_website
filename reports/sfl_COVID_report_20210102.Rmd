...
southFloridaHospitalised_df <- 
	read_csv(
		file = "../../data/ESS_processed/ESS_southFL_summary_20210103.csv"
	)
...
deathsbyday_df <- read_csv(
	"../../data/deaths/FLDH_COVID19_deathsbyday_bycounty_20210103.csv"
) %>% 
	# Deaths are now ten weeks behind
...
The average number of COVID-19 tests performed each day increased from about 2,000 in to in early April to around 12,500 in August. Over the past two weeks, there has been an average increase of `r round(mdRecentTCountSlope_num)` additional tests performed each day in Miami-Dade County. This data is presented on a log scale.
...
There was a general decrease in the number of new positive test results from early April until around mid May when Miami-Dade reopened. Afterwards, this trend flattened out and then increased. Over the past two weeks, there has been an average increase of `r round(mdRecentPCountSlope_num, 2)` additional positive cases each day, per day in Miami-Dade County. 
...
During the 14-day period from `r recentStartEnd_date[1]` to `r recentStartEnd_date[2]`, there was a <b> `r mdAdjective` `r if (mdRecentSlope_num > 0) "increase" else "decrease"` </b> in the proportion of positive COVID-19 cases reported in Miami-Dade County. Over this period, there was an average change of `r round(mdRecentSlope_num, 3)` percentage points per day for new positive cases. Given a flat or increasing number of tests performed, the proportion of positive tests obtained is the **key metric** for examining COVID-19 trajectory.
...
Overall, the proportion of positive test results out of all tests **decreased significantly** from just under 20% in early April to the proportion of about 5% at the end of May. This is function of test availability and the population being tested. **After reopening, the proportion of positive tests increased steadily until late July. Subsequently, there was a consistent decline in positivity rate until mid-September. Recently, the change positivity rate has plateaued.** 
...
**Recently, on October 27th, there were 424 COVID positive patients hospitalized. This was a decrease from a high of 2299 obtained on July 21st. Additionally on October 27th, there were 103 COVID positive patients in the ICU. This was a decrease from from a high of 561 obtained on July 26th. Furthermore, on October 27th, there were 64 COVID positive patients ventilated. This was a decrease from a high of 335 obtained on July 24th. **  Note: data is plotted on a log scale.
...
Percent_Intensive<-miamidadeHospitalised2_df %>% 
	mutate(PctICU = 100 * ICU / Hospitalized) %>%
	mutate(across(where(is.numeric), round, 2))

Percent_Ventilated <-miamidadeHospitalised2_df %>%
...
ggplot(data = miamidadeHospitalised2_df %>% 
		mutate(PctICU = 100 * ICU / Hospitalized) %>% 
		mutate(PctVentICU = 100 * Ventilated / ICU) %>% 
...
Currenly `r Percent_Intensive$PctICU[nrow(Percent_Intensive)]` % of COVID positive patients hospitalized are receiving intensive care. Of the patients recieving intensive care,  `r Percent_Ventilated$PctVentICU[nrow(Percent_Ventilated)]` % are on venatilators.
...
**Note: Due to delays in classifying the cause of death and thus of reporting deaths due to COVID-19, the most recent data will undercount the number of deaths. Specifically, 25% of newly reported deaths were from on or before August 2nd. This date indicates a point where 75% of our data is considered complete. For this reason, approximately the last 13 weeks were excluded from this analysis of deaths.**
...
browCases_df <- read_csv(
	file = "../../data/cases/FLDH_COVID19_cases_broward_20201108.csv"
) %>% 
...
### Total COVID-19 Tests by Day
Over the past two weeks, there has been an average increase of `r round(browRecentTCountSlope_num)` additional tests performed each day in Broward County. This data is presented on a log scale.
...
### Positive COVID-19 Tests by Day
There was a general decrease in the number of new positive test results from early April until around May 11th. Afterwards, this trend flattened out and then increased. Over the past two weeks, there has been a <b> `r browAdjective` `r if (browRecentPCountSlope_num > 0) "increase of" else "decrease of"` </b>  `r round(browRecentPCountSlope_num, 1)` additional positive cases each day, per day in Broward County.
...
**On October 27th, `r browardHospitalised_df$Hospitalized[nrow(browardHospitalised_df)]` COVID-positive patients were hospitalized, this was down from a high of 1442 hospitalized on July 23rd. On October 27th, `r browardHospitalised_df$ICU[nrow(browardHospitalised_df)]` COVID-positive patients were in the ICU, this was down from a high of 291 on July 24th. Finally, on October 27th, `r browardHospitalised_df$Ventilated[nrow(browardHospitalised_df)]` COVID-positive patients were on ventilators, this was down from a high of 223 on July 28th. Overall the past two week, the number in hospitals has been stable.**
...
```{r pb_read_data}
pbCases_df <- read_csv(
	file = "../../data/cases/FLDH_COVID19_cases_palmbeach_20201108.csv"
)
...
### Total COVID-19 Tests by Day
The average number of COVID-19 tests performed each day has increased from about 500 in early April to roughly 5000 in July. Over the past two weeks, there has been an average change of `r round(pbRecentTCountSlope_num)` additional tests performed each day in Palm Beach County. This data is presented on a log scale.
...
### Positive COVID-19 Tests by Day
There was a general decrease in the number of new positive test results from early April until around May 11th. Afterwards, this trend flattened out and then increased. Over the past two weeks, there has been an average change of `r round(pbRecentPCountSlope_num,)` additional positive cases each day, per day in Palm Beach County.
...
### Proportion of Positive Cases within Past Two Weeks
During the 14-day period from `r recentStartEnd_date[1]` to `r recentStartEnd_date[2]`, there was a <b> `r pbAdjective` `r if (pbRecentSlope_num > 0) "increase" else "decrease"` </b> in the proportion of positive COVID-19 cases reported in Palm Beach County. Over this period, there was an average change of `r round(pbRecentSlope_num, 3)` percentage points per day for new positive cases.
...
**On October 27th, there were `r palmbeachHospitalised_df$Hospitalized[nrow(palmbeachHospitalised_df)]` COVID-positive patients hospitalized, this was down from a high of 689 on July 16th. On October 27th, there were `r palmbeachHospitalised_df$ICU[nrow(palmbeachHospitalised_df)]` COVID-positive patients in the ICU, this was down from a high of 151 on July 14th. Finally, On October 27th, there were `r palmbeachHospitalised_df$Ventilated[nrow(palmbeachHospitalised_df)]` COVID-positive patients ventilated, this was down from a reported high of 105 on July 9th. Howver, over the past two weeks, the number in hospitals has increased.**
...
Currently, under 40% of COVID-19 hospitalized individuals are in the ICU. Under 50% of those in the ICU require ventilation.
...
# Conclusion
During the past two weeks, the number of positive COVID-19 tests has increased by an average of `r round(mdRecentPCountSlope_num)`, `r round(browRecentPCountSlope_num)`, and `r round(pbRecentPCountSlope_num)` each day for Miami-Dade, Broward, and Palm Beach Counties, respectively. All three counties experienced increases in cases, tests, case positivity and hospitalization census. All three counties experienced a decline in reported deaths. There is currently a 13 week delay in death reporting for the region.
...
- **COVID-19 Test Results**: The number of positive and negative COVID-19 test results come from the Florida Department of Health: https://urldefense.proofpoint.com/v2/url?u=https-3A__floridahealthcovid19.gov_&d=DwIGaQ&c=lhMMI368wojMYNABHh1gQQ&r=ZzssdlRrRS7QXnhuTSa63A&m=FiYrcPTgc7jv-lrsuxC7WrLfGdMPSiPryOfExF9RPiI&s=48_KO4WJjz8cwft_LHvv6hhMVNXChG3DXIF8zQew5-M&e= .
- **COVID-19 Hospital Census**: The hospitalization data is disseminated by Florida’s Agency for Health Care Administration: https://urldefense.proofpoint.com/v2/url?u=https-3A__ahca.myflorida.com_&d=DwIGaQ&c=lhMMI368wojMYNABHh1gQQ&r=ZzssdlRrRS7QXnhuTSa63A&m=FiYrcPTgc7jv-lrsuxC7WrLfGdMPSiPryOfExF9RPiI&s=k0gnZiyqhunaH7AdFmstPtXIdtAILFsHokXtvNZjYY4&e= .
...
https://urldefense.proofpoint.com/v2/url?u=https-3A__floridahealthcovid19.gov&d=DwIGaQ&c=lhMMI368wojMYNABHh1gQQ&r=ZzssdlRrRS7QXnhuTSa63A&m=FiYrcPTgc7jv-lrsuxC7WrLfGdMPSiPryOfExF9RPiI&s=Gew0b4BHf_xQskxz0Edx77huzU3ZGcbArI47WlNW8bI&e= .